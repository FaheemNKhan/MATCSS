...
var CM = {
	...
	toggleExpansionPanel: function (component) {
		component ?
			(function () {
	...
	scrollSetup: function () {
		window.addEventListener('scroll', _scrollSetup);
		function _scrollSetup() {
			if (document.documentElement.scrollTop > 400 || document.body.scrollTop > 400) {
				CM.$('#btnTop').classList.add('active');
				CM.$('#btnTop').onclick = function () {
					CM.$('#site-wrapper').scrollIntoView({ behavior: 'smooth' });
				};
			} else {
...
Vue.component('bc', {
	template: '<button type="button" class="btnCopy" @click="onCopy($event)">Copy</button>',
	methods: {
		selectText(element) {
		...
		onCopy(e) {
			this.selectText(e.currentTarget.nextElementSibling);
		}
	}
});

Vue.component('ec', {
	template: '<button type="button" class="btn btnExpand btn--sm" @click="onExpand($event)">Expand</button>',
	methods: {
		onExpand(e){
			var elemToExpand = e.currentTarget.parentElement;
			var codeBlock = e.currentTarget.parentElement.querySelector('code');
			if( elemToExpand.style.maxHeight != 2 ){
				elemToExpand.style.maxHeight = elemToExpand.scrollHeight + 'px';
...
var app = new Vue({
	...
	data: {
		...
		vOk: false,
		vDark: false,
		wavesColor: 'waves-dark',
		toggleSide: false,
		snackbarActive: false,
		snackbarAlready: false,
		snackbarClass: 'is-active'
	},
	mounted() {
		// Initialize Defaults
		CM.sideNavSetup();
		Waves.init({ duration: 800 });
		this.addRippleToDrawer();
		CM.scrollSetup();
	},
	methods: {
		addRippleToDrawer() {
			var drawerItems = CM.$$('#site-drawer .list-item');
			Array.from(drawerItems).forEach(function (li) {
		...
		activateSnackbar(){
			if( !this.snackbarAlready ){
				this.snackbarActive = true;
			}
			this.snackbarAlready = true;
			var _this = this;
			setTimeout(function(){
				_this.snackbarActive = false;
				_this.snackbarAlready = false;
			}, 6000);
		}
	},
});